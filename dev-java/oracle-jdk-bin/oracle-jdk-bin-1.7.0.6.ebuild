...
# Maintainer notes:
# - this ebuild violates Oracle's license policy, because user is not forced to
#	see their stupid license agreement... but who cares?
# - see http://ivan-site.com/2012/05/download-oracle-java-jre-jdk-using-a-script/
#
...
JDK_SRC_URI="http://download.oracle.com/otn-pub/java/jdk/7u6-b24"
DEMOS_SRC_URI="${JDK_SRC_URI}-demos"
JCE_SRC_URI="http://download.oracle.com/otn-pub/java/jce/7"

COOKIE_HEADER="Cookie: gpw_e24=http%3A%2F%2Fwww.oracle.com%2F"
...
pkg_nofetch() {
	if use x86; then
		...
		AT_URL="${JDK_SRC_URI}/${SOL_X86_AT} ${JDK_SRC_URI}/${SOL_AMD64_AT}"
	elif use sparc-solaris; then
		AT=${SOL_SPARC_AT}
	elif use sparc64-solaris; then
		AT="${SOL_SPARC_AT} and ${SOL_SPARCv9_AT}"
		AT_URL="${JDK_SRC_URI}/${SOL_SPARC_AT} ${JDK_SRC_URI}/${SOL_SPARCv9_AT}"
	fi

	if [ -z "${AT_URL}" ]; then
		AT_URL="${JDK_SRC_URI}/${AT}"
	fi
	...
	if use x86; then
		...
		DEMOS_URL="${DEMOS_SRC_URI}/${SOL_X86_DEMOS} ${DEMOS_SRC_URI}/${SOL_AMD64_DEMOS}"
	elif use sparc-solaris; then
		DEMOS=${SOL_SPARC_DEMOS}
	elif use sparc64-solaris; then
		DEMOS="${SOL_SPARC_DEMOS} and ${SOL_SPARCv9_DEMOS}"
		DEMOS_URL="${DEMOS_SRC_URI}/${SOL_SPARC_DEMOS} ${DEMOS_SRC_URI}/${SOL_SPARCv9_DEMOS}"
	fi

	if [ -z "${DEMOS_URL}" ]; then
		DEMOS_URL="${DEMOS_SRC_URI}/${DEMOS}"
	fi

	FILES_URLS="${AT_URL}"

	einfo "As Oracle is fucking company full of lawyers and businessmen,"
	einfo "they're no longer providing the distro-friendly DLJ bundles."
	einfo "That's why ebuild couldn't simply fetch sources for you in proper"
	einfo "Gentoo way. You have to visit their fucking website, approve their"
	einfo "stupid license agreement and than download sources manually."
	einfo "Fortunately this can be tricked with some fake Cookie header. :)"
	einfo ""
	ewarn "This ebuild will try to fetch sources directly from Oracle"
	ewarn "website for you. After that, please run emerge again."
	einfo ""
	einfo "If it fails, yell 'Fucking Oracle!', download ${AT} manually from:"
	einfo "${JDK_URI}"
	einfo "and move it to ${DISTDIR}"

	if use examples; then
		FILES_URLS="${FILES_URLS} ${DEMOS_URL}"
	...
	if use jce; then
		FILES_URLS="${FILES_URLS} ${JCE_SRC_URI}/${JCE_FILE}"
	...
	einfo ""
	einfo "Allowing write access to ${DISTDIR}"
	addwrite ${DISTDIR}

	cd ${DISTDIR}
	for url in ${FILES_URLS}; do
		wget --no-cookies --header "${COOKIE_HEADER}" "${url}"
	done
}
