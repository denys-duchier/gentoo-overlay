...
HTTP_PASSENGER_MODULE_PV="3.0.11"
...
src_prepare() {
	...
	if use nginx_modules_http_passenger; then
		cd "${WORKDIR}"/${HTTP_PASSENGER_MODULE_P}
		epatch \
			"${FILESDIR}/passenger-3.0.11-ldflags.patch" \
...
src_configure() {
	...
	if use nginx_modules_http_passenger; then
		http_enabled=1
		myconf+=" --add-module=${WORKDIR}/${HTTP_PASSENGER_MODULE_P}/ext/nginx"
	fi
